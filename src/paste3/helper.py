...
def to_dense_array(X): return np.array(X.todense()) if isinstance(X, scipy.sparse.csr.spmatrix) else X
def extract_data_matrix(adata, rep): return adata.X if rep is None else adata.obsm[rep]
